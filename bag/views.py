    ...
    if size:
        ...
        if item_id in list(bag.keys()):
            ...
            if size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(request, f'Updated size {size} {product.title} quantity to {bag[item_id]["items_by_size"][size]}')
            else:
                bag[item_id]['items_by_size'][size] = quantity
                messages.success(request, f'Added size {size} {product.title}\
                to your bag')
        else:
            bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(request, f'Added size {size}\
            {product.title} to your bag')
    else:
...
def adjust_bag(request, item_id):
    ...
    if size:
        if quantity > 0:
            bag[item_id]['items_by_size'][size] = quantity
            messages.success(request, f'Updated size {size} {product.title}\
            quantity to {bag[item_id]["items_by_size"][size]}')
        else:
            del bag[item_id]['items_by_size'][size]
            if not bag[item_id]['items_by_size']:
                bag.pop(item_id)
            messages.success(request, f'Removed size {size} {product.title}\
            from your bag')
    else:
        if quantity > 0:
            bag[item_id] = quantity
...
def remove_from_bag(request, item_id):
    ...
    try:
        ...
        if size:
            ...
            if not bag[item_id]['items_by_size']:
                bag.pop(item_id)
            messages.success(request, f'Removed size {size}\
            {product.title} from your bag')
        else:
            bag.pop(item_id)
            messages.success(request, f'Removed {product.title} from your bag')
